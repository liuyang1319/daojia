...
final class WMCountDown {
    ...
    /// 回调 计算好的 天、时、分、秒
    var countDown: (( _ minute: String, _ second: String) -> Void)?
    /// 开始启动定时器，需传入结束时间
    ...
    fileprivate func createTimer() {
        ...
        // 当时间小于当前时间
        if timeCount <= 0 { countDown("00", "00") ; return }
        // 设置计算时间的格式
    ...
    fileprivate func calculateCurrentDate() {
        ...
        if  currentDate!.second! == 0
            ...
            && currentDate!.day! == 0 {
            timer!.cancel()
            DispatchQueue.main.async(execute: { countDown("00", "00") })
            return
        }
        // 主线程更新返回数据
        DispatchQueue.main.async(execute: { [unowned self ] in
            countDown(String(format: "%02d", self.currentDate!.minute!),
