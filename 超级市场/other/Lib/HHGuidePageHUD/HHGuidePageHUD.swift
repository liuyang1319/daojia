...
//        self.addSubview(skipButton)
    ...
    @objc func skipButtonClick() -> Void {
        NotificationCenter.default.post(name: NSNotification.Name("HHGuidePageHUDHomeController"), object: self, userInfo:nil)
    ...
    func addImages() -> Void {
        ...
        for i in 0..<imageArray.count {
            ...
            if i == imageArray.count - 1 {
                ...
                if isIphoneX == true {
                    let startButton = UIButton.init(frame: CGRect.init(x:0, y:LBFMScreenHeight-75-34, width: HHScreenWidth, height:50))
//                    startButton.backgroundColor = UIColor.red
                    ...
                    let startButton = UIButton.init(frame: CGRect.init(x:0, y:LBFMScreenHeight-90, width: HHScreenWidth, height:50))
//                    startButton.backgroundColor = UIColor.red
                    //                startButton.setBackgroundImage(UIImage.init(named: "guide_btn_bg"), for: .normal)
                    startButton.addTarget(self, action: #selector(skipButtonClick), for: .touchUpInside)
                    imageView.addSubview(startButton)
