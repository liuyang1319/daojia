...
extension YDShopCartViewModel {
    ...
    func refreshGoodsCart() {
        requestData(object: nil)
    }
...
//    }
//
    func refreshGoodsCartAllSelect() {
        requestData(object: true)
    }
    
    private func requestData(object: Any?) {
        
        let uuid = YDInfoTool.getUUid()
        let memberId = YDInfoTool.getMemberId()
        YDShopCartViewProvider.request(.getShoppingCartListInfo(deviceNumber: uuid, memberId: memberId)) { result in
            if case let .success(response) = result {
                //解析数据
                let data = try? response.mapJSON()
                if data != nil{
                    ...
                    NotificationCenter.default.post(name: NSNotification.Name.init(kShopCartDataRefresh), object: object)
                }
            }
        }
    }
