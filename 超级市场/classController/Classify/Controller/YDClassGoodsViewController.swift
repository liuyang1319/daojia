//
//  YDClassGoodsViewController.swift
//  超级市场
//
...
import Alamofire
class YDClassGoodsViewController: YDBasicViewController,UITextFieldDelegate{
...
//        field.leftView = searchBtn
    ...
    lazy var searchGoodsViewModel: YDClassGoodsViewModel = {
        return YDClassGoodsViewModel()
    }()
...
//            self.countSum.text = String(format: "约%d个结果", self.searchGoodsViewModel.searchGoodsListModel?.count ?? 0)
        ...
        searchGoodsViewModel.refreshClassifyGoodsDataSource(id:self.typeID )
    }
    //    搜索商品名
    func requestSearchNameGoodsDate(){
        searchGoodsViewModel.updateDataBlock = { [unowned self] in
            //            self.tableView.uHead.endRefreshing()
            self.countSum.text = String(format: "约%d个结果", self.searchGoodsViewModel.categorylistModel?.count ?? 0)
            // 更新列表数据
            self.tableView.reloadData()
        }
...
//            self.tableView.uHead.endRefreshing()
        ...
        searchGoodsViewModel.refreshClassfiyRankGoodsDataSource(rank:"count", id: self.typeID)
...
//                self.tableView.uHead.endRefreshing()
           ...
           searchGoodsViewModel.refreshClassfiyRankGoodsDataSource(rank:"desc", id: self.typeID)
        }else{
...
//                self.tableView.uHead.endRefreshing()
            ...
            searchGoodsViewModel.refreshClassfiyRankGoodsDataSource(rank:"asc", id: self.typeID)
        }
...
//            self.tableView.uHead.endRefreshing()
        ...
        searchGoodsViewModel.refreshClassfiyRankGoodsDataSource(rank:"saleNums", id: self.typeID)
    }
...
extension  YDClassGoodsViewController: UITableViewDataSource, UITableViewDelegate{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.seearchGoodsModel = searchGoodsViewModel.categorylistModel?[indexPath.row]
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let goodsModel = searchGoodsViewModel.categorylistModel?[indexPath.row]
...
extension  YDClassGoodsViewController:YDClassGoodsTableViewCellDelegate{
    ...
    func plusSelctTableViewCell(button: UIButton, cell: YDClassGoodsTableViewCell, icon: UIImageView) {
        ...
        //添加到已购买数组之中
        self.addGoodArray.append(searchGoodsViewModel.categorylistModel![button.tag])
        let goodsModel = searchGoodsViewModel.categorylistModel![button.tag]
