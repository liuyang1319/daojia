//
//  YDClassAllGoodsViewController.swift
//  超级市场
//
...
import Alamofire
class YDClassAllGoodsViewController: YDBasicViewController,UITextFieldDelegate{
    ...
    var typeID = String()
    let YDClassGoodsTableViewCellID = "YDClassGoodsTableViewCell"
...
//        tableView.uHead = URefreshHeader{ [weak self] in self?.requestSearchGoodsDate() }
        tableView.register(YDClassGoodsTableViewCell.self, forCellReuseIdentifier: YDClassGoodsTableViewCellID)
...
//        field.leftView = searchBtn
    ...
    lazy var searchGoodsViewModel: YDClassAllGoodsViewModel = {
        return YDClassAllGoodsViewModel()
    }()
...
//        self.searchView.addSubview(countSum)
        ...
        self.searchField.text = self.searchName
...
//    去购物车
    ...
    func requestSearchGoodsDate(){
        ...
        searchGoodsViewModel.refreshClassifyGoodsDataSource(id:self.typeID,sort:"count" )
    }
    //    搜索商品名
    func requestSearchNameGoodsDate(){
        searchGoodsViewModel.updateDataBlock = { [unowned self] in
            //            self.tableView.uHead.endRefreshing()
            self.countSum.text = String(format: "约%d个结果", self.searchGoodsViewModel.categoryAllListModel?.count ?? 0)
            // 更新列表数据
            self.tableView.reloadData()
        }
...
//                self.tableView.uHead.endRefreshing()
           ...
           searchGoodsViewModel.refreshClassifyGoodsDataSource(id:self.typeID,sort:"desc" )
//            refreshClassfiyRankGoodsDataSource(rank:"desc", id: self.typeID)
        }else{
...
//                self.tableView.uHead.endRefreshing()
            ...
            searchGoodsViewModel.refreshClassifyGoodsDataSource(id:self.typeID,sort:"asc" )
//            refreshClassfiyRankGoodsDataSource(rank:"asc", id: self.typeID)
        }
...
//            self.tableView.uHead.endRefreshing()
        ...
        searchGoodsViewModel.refreshClassifyGoodsDataSource(id:self.typeID,sort:"saleNums" )
//        refreshClassfiyRankGoodsDataSource(rank:"saleNums", id: self.typeID)
    }
...
extension  YDClassAllGoodsViewController: UITableViewDataSource, UITableViewDelegate{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell:YDClassGoodsTableViewCell = tableView.dequeueReusableCell(withIdentifier: YDClassGoodsTableViewCellID, for: indexPath) as! YDClassGoodsTableViewCell
        cell.seearchGoodsModel = searchGoodsViewModel.categoryAllListModel?[indexPath.row]
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let goodsModel = searchGoodsViewModel.categoryAllListModel?[indexPath.row]
...
extension  YDClassAllGoodsViewController:YDClassGoodsTableViewCellDelegate{
    ...
    func plusSelctTableViewCell(button: UIButton, cell: YDClassGoodsTableViewCell, icon: UIImageView) {
        ...
        //添加到已购买数组之中
        self.addGoodArray.append(searchGoodsViewModel.categoryAllListModel![button.tag])
        let goodsModel = searchGoodsViewModel.categoryAllListModel![button.tag]
