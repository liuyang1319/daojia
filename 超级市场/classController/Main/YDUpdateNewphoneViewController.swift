//
//  YDUpdateNewphoneViewController.swift
//  超级市场
//
//  Created by 王林峰 on 2019/8/22.
...
class YDUpdateNewphoneViewController: YDBasicViewController {
    ...
    private lazy var finishLabel : UIButton = {
        ...
        button.setTitleColor(UIColor.white, for: UIControl.State.normal)
        button.clipsToBounds = true
        button.setTitle("绑定新手机", for: UIControl.State.normal)
    ...
    override func viewDidLoad() {
        ...
        self.iphoneField.frame = CGRect(x: 30, y: 104, width:LBFMScreenWidth-60, height: 35)
    ...
    @objc func codeFinishButtonClick() {
        ...
        YDUserCenterProvider.request(.setUpdateUserCenterNewPhoneInfo(newPhone: self.iphoneField.text! as NSString) ){ result in
    ...
    @objc func finishIphoneButtonClick(){
        ...
        YDUserCenterProvider.request(.setUpdateUserCenterNewPhoneVerifyInfo(newPhone: self.iphoneField.text!, newPhoneCode: self.codeField.text!, token: UserDefaults.LoginInfo.string(forKey:.token)! as String, memberPhone: UserDefaults.LoginInfo.string(forKey:.phone)! as String) ){ result in
            if case let .success(response) = result {
                let data = try? response.mapJSON()
                let json = JSON(data!)
                if json["success"] == true{
