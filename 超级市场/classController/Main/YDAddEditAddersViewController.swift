//
//  YDAddEditAddersViewController.swift
//  超级市场
//
...
class YDAddEditAddersViewController: YDBasicViewController,YDSearchAddersViewControllerDelegate,YDSelectCityViewControllerDelegate,UITextFieldDelegate {

    var type = NSString() //1.公司 2.家  3.学校
    var isDefault = NSString() //0 非默认  1.默认
    var sexType = NSString() //1.男 2.女
    var addressId = String()    //唯一地址表标示
    var addressCode = String()  //区域code
     var cityStr = String() //城市
    var streetStr = String()//区域
    //    经纬度
...
//    var code = String()
    ...
    private lazy var iphoneField:UITextField = {
        ...
        textField.backgroundColor = UIColor.white
        // 提示语
        textField.text = UserDefaults.LoginInfo.string(forKey: .phone)
        textField.layer.cornerRadius = 5
        textField.clipsToBounds = true
    ...
    private lazy var cityButton: UIButton = {
        ...
        button.addTarget(self, action: #selector(selectMadamButtonClick), for: UIControl.Event.touchUpInside)
        return button
    }()
    ...
    private lazy var numberField:UITextField = {
        ...
        textField.delegate = self
        // 提示语
        textField.placeholder = "请输入楼单元门牌号"
        textField.layer.cornerRadius = 5
        textField.clipsToBounds = true
        textField.font = UIFont.systemFont(ofSize: 14, weight: UIFont.Weight.bold)
        // 完成按钮样式
    ...
    override func viewDidLoad() {
        ...
        self.selectMadam.frame = CGRect(x: self.takeLabel.frame.maxX, y: self.line.frame.maxY+15, width: 20, height: 20)
        ...
        self.selectMan.frame = CGRect(x: self.MadamLabel.frame.maxX+50, y: self.line.frame.maxY+15, width: 20, height: 20)
...
//        self.view.addSubview(self.line4)
        ...
        self.selectArea.frame = CGRect(x: self.areaLabel.frame.maxX, y: self.line3.frame.maxY+15, width: LBFMScreenWidth-140, height: 20)
    ...
    var categoryContentsModel:YDAddAddersModel? {
        didSet {
            ...
            self.iphoneField.text = model.phone as String?
            self.selectCity.text =  model.addressRegion as String?
            self.selectArea.text = model.street as String?
            self.numberField.text = model.doorNumber as String?
            
            if (model.type?.isEqual("1"))! {
                self.type = "1"
                self.companyButton.isSelected = true
                self.companyButton.setTitleColor(YMColor(r: 255, g: 140, b: 43, a: 1), for: UIControl.State.normal)
                self.companyButton.setBackgroundImage(UIImage(named: "type_image"), for: UIControl.State.normal)
            }else if (model.type?.isEqual("2"))! {
                self.type = "2"
                self.homeButton.isSelected = true
                self.homeButton.setTitleColor(YMColor(r: 255, g: 140, b: 43, a: 1), for: UIControl.State.normal)
                self.homeButton.setBackgroundImage(UIImage(named: "type_image"), for: UIControl.State.normal)
            }else if (model.type?.isEqual("3"))! {
                self.type = "3"
                self.schoolButton.isSelected = true
                self.schoolButton.setTitleColor(YMColor(r: 255, g: 140, b: 43, a: 1), for: UIControl.State.normal)
                self.schoolButton.setBackgroundImage(UIImage(named: "type_image"), for: UIControl.State.normal)
            }
            if (model.isDefault?.isEqual("0"))! {
                self.onSelect.isOn = false
                self.isDefault = "0"
            }else if (model.isDefault?.isEqual("1"))!{
                self.onSelect.isOn = true
                self.isDefault = "1"
            }
            
            self.addressId = model.id ?? ""
            self.addressCode = model.addressCode ?? ""
...
// 选择区域
    func returnAddersValue(city:String,adders: String, longitude: CGFloat, latitude: CGFloat, code: String){
        ...
        self.addressCode = code
        self.cityStr = city
        self.selectArea.text = adders
//        self.streetStr = adders
    ...
    @objc func selectcompanyButtonClick(company:UIButton){
        if company.isSelected == true{
            ...
            company.setBackgroundImage(UIImage(named: "type_image_n"), for: UIControl.State.normal)
        }else{
            self.homeButton.setTitleColor(YMColor(r: 51, g: 51, b: 51, a: 1), for: UIControl.State.normal)
            self.homeButton.setBackgroundImage(UIImage(named: "type_image_n"), for: UIControl.State.normal)
            self.schoolButton.setTitleColor(YMColor(r: 51, g: 51, b: 51, a: 1), for: UIControl.State.normal)
            self.schoolButton.setBackgroundImage(UIImage(named: "type_image_n"), for: UIControl.State.normal)
            self.homeButton.isSelected = false
            self.schoolButton.isSelected = false
    ...
    @objc func selecthomeButtonClick(home:UIButton){
        if home.isSelected == true{
            ...
            home.setBackgroundImage(UIImage(named: "type_image_n"), for: UIControl.State.normal)
        }else{
            self.companyButton.setTitleColor(YMColor(r: 51, g: 51, b: 51, a: 1), for: UIControl.State.normal)
            self.companyButton.setBackgroundImage(UIImage(named: "type_image_n"), for: UIControl.State.normal)
            self.schoolButton.setTitleColor(YMColor(r: 51, g: 51, b: 51, a: 1), for: UIControl.State.normal)
            self.schoolButton.setBackgroundImage(UIImage(named: "type_image_n"), for: UIControl.State.normal)
            self.companyButton.isSelected = false
            self.schoolButton.isSelected = false
    ...
    @objc func selectschoolButtonClick(school:UIButton){
        if school.isSelected == true{
            ...
            school.setBackgroundImage(UIImage(named: "type_image_n"), for: UIControl.State.normal)
        }else{
            self.homeButton.setTitleColor(YMColor(r: 51, g: 51, b: 51, a: 1), for: UIControl.State.normal)
            self.homeButton.setBackgroundImage(UIImage(named: "type_image_n"), for: UIControl.State.normal)
            self.companyButton.setTitleColor(YMColor(r: 51, g: 51, b: 51, a: 1), for: UIControl.State.normal)
...
//        let numberStr:String = String.init(data:numberDate, encoding:String.Encoding.utf8) ?? ""
        YDUserAddersProvider.request(.setUpdateEditAdders(name:self.nameField.text ?? "", type: self.type as String, isDefault: self.isDefault as String,addressRegion:self.cityStr, addressId:self.addressId as String,  sex: self.sexType as String, token:UserDefaults.LoginInfo.string(forKey: .token)! as String, memberPhone: UserDefaults.LoginInfo.string(forKey: .phone)! as String,addressCode:self.addressCode as String,longitude:self.longitude,latitude:self.latitude,phone:self.iphoneField.text! as String,street:self.selectArea.text! as String,doorNumber:self.numberField.text ?? "")) { result  in
