//
//  YDsetPasswordViewController.swift
//  超级市场
//
...
class YDsetPasswordViewController: YDBasicViewController,UITextFieldDelegate {
    ...
    @objc func loginIphoneButtonClick(){
        ...
        YDUserLoginProvider.request(.setUserUpdatePassword(
            memberPhone: self.iphoneField.text! as NSString,
            password: self.passworField.text! as NSString,
            token: UserDefaults.LoginInfo.string(forKey: .token)! as NSString
        )) { result  in
            if case let .success(response) = result {
                let data = try? response.mapJSON()
                let json = JSON(data!)
