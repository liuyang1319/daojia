//
//  YDOrderPayViewController.swift
//  超级市场
//
...
import MJRefresh
class YDOrderPayViewController: YDBasicViewController {
...
//        tableView.uHead = URefreshHeader{ [weak self] in self?.requestClassifyGoodsDate() }
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        header.setRefreshingTarget(self, refreshingAction: #selector(YDOrderPayViewController.headerRefresh))
    ...
    func requestClassifyGoodsDate(){
        if  isUserLogin() != true{
            ...
            self.orderListViewModel.refreshDataSource(typeState:"pay")
        }
    }
    //    查询待支付订单
...
//    func outoCancelGoodsOrder(problemId:Int,orderNum:String){
//        YDShopCartViewProvider.request(.getOrderGoodCancelListInfo(orderNum:orderNum, problemId: problemId, token: (UserDefaults.LoginInfo.string(forKey: .token)! as NSString) as String, memberPhone:(UserDefaults.LoginInfo.string(forKey: .phone)! as NSString) as String)) { result in
//            if case let .success(response) = result {
//                let data = try? response.mapJSON()
//                let json = JSON(data!)
//                print("------------------%@",json)
...
extension YDOrderPayViewController : UITableViewDelegate, UITableViewDataSource {
...
extension YDOrderPayViewController: YDCancelCauseViewControllerDelegate {
...
//    
//}
extension YDOrderPayViewController : YDOrderListTableViewCellDelegate{
    // MARK: --------------   取消订单
...
extension YDOrderPayViewController {
