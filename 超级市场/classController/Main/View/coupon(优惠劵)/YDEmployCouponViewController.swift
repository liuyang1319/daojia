//
//  YDEmployCouponViewController.swift
//  超级市场
//
...
import HandyJSON
import MJRefresh
class YDEmployCouponViewController: YDBasicViewController {
...
//        tableView.uHead = URefreshHeader{ [weak self] in self?.requestSearchGoodsDate() }
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.title = "我的优惠劵"
        header.setRefreshingTarget(self, refreshingAction: #selector(YDEmployCouponViewController.headerRefresh))
    ...
    func requestSearchGoodsDate(){
        ...
        couponListModel.refreshCouponDataSource(status: "2", token:  (UserDefaults.LoginInfo.string(forKey: .token)! as NSString) as String, memberPhone: (UserDefaults.LoginInfo.string(forKey: .phone)! as NSString) as String)
    }
    //    为你推荐
...
extension YDEmployCouponViewController : UITableViewDelegate, UITableViewDataSource {
...
extension YDEmployCouponViewController : YDHomeRecommendViewDelegate{
