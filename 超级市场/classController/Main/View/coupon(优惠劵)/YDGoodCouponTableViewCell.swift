//
//  YDGoodCouponTableViewCell.swift
//  超级市场
//
...
class YDGoodCouponTableViewCell: UITableViewCell {
//    低图
    ...
    var couponContentsModel:YDCouponDetailGoodsModel? {
        didSet {
            ...
            if model.status == "1"{
                ...
                self.backImage.image = UIImage(named: "coupon_tag_used")
                self.iconImage.kf.setImage(with: URL(string: model.imageUrl ?? ""),placeholder:UIImage(named: "coupon_used_bg"))
            }else if model.status == "3"{
                self.backImage.image = UIImage(named: "coupon_tag_out")
                 self.iconImage.kf.setImage(with: URL(string: model.imageUrl ?? ""),placeholder:UIImage(named: "coupon_used_bg"))
            }
