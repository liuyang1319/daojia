//
//  YDUserAddInvoiceViewController.swift
//  超级市场
//
...
class YDUserAddInvoiceViewController: YDBasicViewController,UITextFieldDelegate,UIAlertViewDelegate{
    ...
    private lazy var mainButton : UIButton = {
        ...
        button.addTarget(self, action: #selector(addMainInvoiceButtonClick), for: UIControl.Event.touchUpInside)
    ...
    private lazy var unitButton : UIButton = {
        ...
        button.clipsToBounds = true
        button.titleLabel?.font = UIFont.systemFont(ofSize: 13)
        button.addTarget(self, action: #selector(addUnitInvoiceButtonClick), for: UIControl.Event.touchUpInside)
        button.setTitle("单位", for: UIControl.State.normal)
        button.layer.borderColor = YMColor(r: 238, g: 238, b: 238, a: 1).cgColor
        button.layer.borderWidth = 1
        button.setTitleColor(YMColor(r: 102, g: 102, b: 102, a: 1), for: UIControl.State.normal)
        button.setTitleColor(YDLabelColor, for: UIControl.State.selected)
...
//        textField.clearButtonMode = UITextField.ViewMode.whileEditing
    ...
    override func viewDidLoad() {
        ...
        self.mainButton.isSelected = true
        self.mainButton.layer.borderColor = YDLabelColor.cgColor
...
//    取消
    ...
    func setAddUserInvoiceView(){
        ...
        self.backView.addSubview(self.mainButton)
        self.mainButton.frame = CGRect(x: self.titleLabel.frame.maxX+15, y: 15, width: 115, height: 30)
        
        self.backView.addSubview(self.unitButton)
        self.unitButton.frame = CGRect(x: self.mainButton.frame.maxX+15, y: 15, width: 115, height: 30)
        
        self.backView.addSubview(self.line)
        self.line.frame = CGRect(x: 0, y: 59, width: LBFMScreenWidth, height: 1)
        
        self.view.addSubview(self.back1View)
        self.back1View.frame = CGRect(x: 0, y: self.backView.frame.maxY, width: LBFMScreenWidth, height: 40)
        
        self.back1View.addSubview(self.titleName)
        self.titleName.frame = CGRect(x: 15, y: 10, width: 30, height: 20)
        
        self.back1View.addSubview(self.line7)
        self.line7.frame = CGRect(x: 0, y: 39, width: LBFMScreenWidth, height: 1)
        
        self.back1View.addSubview(self.titleField)
        self.titleField.frame = CGRect(x: 85, y: 10, width: 200, height: 20)
...
//    个人
    @objc func addMainInvoiceButtonClick(){
        self.back2View.isHidden = true
        self.back1View.isHidden = false
        if self.mainButton.isSelected == true {
            self.mainButton.isSelected = false
            self.mainButton.layer.borderColor = YMColor(r: 238, g: 238, b: 238, a: 1).cgColor
        }else{
            self.mainButton.layer.borderColor = YDLabelColor.cgColor
            self.unitButton.layer.borderColor = YMColor(r: 238, g: 238, b: 238, a: 1).cgColor
            self.mainButton.isSelected = true
            self.unitButton.isSelected = false
        }
    }
//    单位
    @objc func addUnitInvoiceButtonClick(){
        self.back1View.isHidden = true
        self.back2View.isHidden = false
        if self.unitButton.isSelected == true {
            self.unitButton.isSelected = false
            self.unitButton.layer.borderColor = YMColor(r: 238, g: 238, b: 238, a: 1).cgColor
        }else{
            self.unitButton.layer.borderColor = YDLabelColor.cgColor
            self.mainButton.layer.borderColor = YMColor(r: 238, g: 238, b: 238, a: 1).cgColor
            self.unitButton.isSelected = true
            self.mainButton.isSelected = false
        }
    }
}
